...
async function rechargeClientQuota(clientId, userId, rechargeData) {
  try {
    ...
    // 🔥 Transaction gérée par le helper existant : BEGIN/COMMIT/ROLLBACK
    // et client.release() automatiques, y compris en cas d'erreur.
    ...
    // 🔥 SYNC REDIS — placée APRÈS la transaction, uniquement si elle a réussi.
    try {
      await redisConnection.incrby(`quota:client:${clientId}`, quantity);
      await redisConnection.expire(`quota:client:${clientId}`, 60 * 60 * 24 * 2);
    } catch (redisErr) {
      logger.warn('Erreur sync Redis recharge quota (non bloquant):', redisErr.message);
    }
