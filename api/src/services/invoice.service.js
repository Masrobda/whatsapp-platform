...
async function confirmPurchase(orderId, userId) {
  ...
  try {
    ...
    // 🔥 SYNC REDIS — placé APRÈS le COMMIT pour garantir que le crédit
    // en base est définitivement acquis avant de le refléter dans Redis.
    try {
      await redisConnection.incrby(`quota:client:${order.client_id}`, order.quantity || 0);
      await redisConnection.expire(`quota:client:${order.client_id}`, 60 * 60 * 24 * 2);
    } catch (redisErr) {
      logger.warn('Erreur sync Redis après achat (non bloquant):', redisErr.message);
    }
