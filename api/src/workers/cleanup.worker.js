    ...
    if (deletedCount > 0) {
      ...
      await query(
        `INSERT INTO audit_logs (action, entity_type, entity_id, new_values)
         VALUES ($1, $2, $3, $4)`,
        ['MESSAGES_CLEANUP', 'message', SYSTEM_UUID, JSON.stringify({ deleted_count: deletedCount })]
      );
    } else {
...
/**
 * Supprimer les fichiers archivés (dans /var/socadel/archive) de plus de N jours
 * Par défaut 60 jours, modifiable via ARCHIVE_RETENTION_DAYS
 */
async function cleanupArchivedFiles() {
  const retentionDays = parseInt(process.env.ARCHIVE_RETENTION_DAYS) || 60;
  const archiveDir = '/var/socadel/archive';
  const fs = require('fs');
  const path = require('path');

  if (!fs.existsSync(archiveDir)) {
    logger.info('Dossier d\'archive introuvable, skip');
  ...
  logger.info(`${deletedCount} fichiers archivés supprimés (plus de ${retentionDays} jours)`);
  return { success: true, deletedCount };
}

/**
 * Nettoyer les factures de plus de 60 jours dans invoices_bot
 */

async function cleanupOldInvoices() {
  const retentionDays = parseInt(process.env.INVOICE_RETENTION_DAYS) || 60;
  ...
  try {
    ...
    do {
      const result = await query(
        `DELETE FROM invoices_bot
         WHERE id IN (
           SELECT id FROM invoices_bot
           WHERE date_creation < NOW() - INTERVAL '${retentionDays} days'
           LIMIT $1
         )
         RETURNING id`,
        [BATCH_SIZE]
      ...
      if (deleted > 0 && deleted === BATCH_SIZE) {
        // Pause courte pour libérer les verrous (10 ms)
        await new Promise(resolve => setTimeout(resolve, 10));
      }
    } while (deleted === BATCH_SIZE);

    logger.info(`${totalDeleted} factures supprimées (> ${retentionDays} jours)`);
    return { success: true, deletedCount: totalDeleted };
  } catch (error) {
    logger.error('Erreur nettoyage invoices_bot:', error);
    return { success: false, error: error.message };
  }
}
