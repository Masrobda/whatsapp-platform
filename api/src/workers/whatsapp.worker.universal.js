...
const { canSendToRecipient, incrementClientStats, reserveQuotaAtomic, refundQuota } = require('../services/message.service');
...
async function processMessage(job) {
  ...
  try {
    ...
    // ── 2bis. 🔥 RÉSERVATION ATOMIQUE DU QUOTA (Redis + fallback Postgres)
    // C'est ICI, juste avant l'envoi réel via WATI, que le quota doit être
    // consommé — c'est le seul chemin de code qui envoie réellement les
    // messages (sendMessage() dans l'API ne fait qu'empiler dans la queue).
    // ── 2bis. Réservation atomique du quota — SEULEMENT pour les messages
// individuels (hors campagne). Les messages de campagne ont déjà eu
// leur quota réservé en lot au moment du dispatch (campaign-dispatch.worker.js).
if (!job.data.is_campaign) {
  const quotaResult = await reserveQuotaAtomic(client_id);
  if (!quotaResult.ok) {
    const reason = quotaResult.reason === 'QUOTA_EXHAUSTED'
      ? 'Quota épuisé : message annulé avant envoi.'
      : 'Client introuvable.';
    logger.warn(`[WORKER] Quota refusé pour ${client_id}: ${reason}`);
    await markMessageAsFailed(messageId, client_id, reason);
    return { success: false, fatal: true, reason };
  }
  quotaReserved = true;
  quotaSource = quotaResult.source;
    ...
    // ── 4. Mise à jour BDD + stats
    await markMessageAsSent(messageId, client_id, result);
    await incrementClientStats(phoneNumber, client_id);

        // ── Contexte bouton "Recevoir ma facture" ──
    try {
      const invoiceShortcut = require('../services/invoice-shortcut.service');
      const tplName = job.data.template_name;
      if (
        job.data.message_type === 'template' &&
        invoiceShortcut.isInvoiceNotifyTemplate(tplName)
      ) {
        const { name, contract } = invoiceShortcut.extractContractAndName(
          job.data.template_params,
          job.data.ordered_values
        );
        if (contract) {
          await invoiceShortcut.registerInvoiceButtonContext({
          ...
          logger.warn(
            `[INVOICE-SHORTCUT] Template notif sans contrat détecté messageId=${messageId}`
          );
        }
      }
    } catch (e) {
      logger.warn(`[INVOICE-SHORTCUT] register pending: ${e.message}`);
    }
    ...
    // 🔥 Remboursement : le message n'a pas été envoyé avec succès,
    // on restitue le quota consommé (que ce soit un échec définitif
    // ou un échec qui va être retenté par BullMQ).
    if (quotaReserved && !job.data.is_campaign) {
  await refundQuota(client_id, quotaSource);
  quotaReserved = false;
}
    ...
    throw error; // BullMQ retente selon backoff — la prochaine tentative réservera à nouveau
  }
}
