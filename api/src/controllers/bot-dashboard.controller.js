...
async listInvoiceSends(request, reply) {
  try {
    const { search, status, source, from, to, page, limit } = request.query;
    const result = await botDashboardService.listInvoiceSends({
      search: search || null,
      status: status || null,
      source: source || null,
      from: from || null,
    ...
    logger.error('[BotDashboard] listInvoiceSends:', error.message);
    return reply.code(500).send({ success: false, error: 'Erreur liste envois factures' });
  }
}

async getInvoiceSendStats(request, reply) {
  try {
    const days = request.query.days ? parseInt(request.query.days) : 30;
    const stats = await botDashboardService.getInvoiceSendStats({ days });
    return reply.code(200).send({ success: true, stats });
  } catch (error) {
    logger.error('[BotDashboard] getInvoiceSendStats:', error.message);
    return reply.code(500).send({ success: false, error: 'Erreur stats envois' });
  }
}

async exportInvoiceSends(request, reply) {
  try {
    const { search, status, source, from, to } = request.query;
    const rows = await botDashboardService.exportInvoiceSends({
      search, status, source, from, to,
    });
    let csv =
      'Contrat,Client,Téléphone,N° facture,Montant,Mois,Template,Langue,Statut,Source,Envoyé le,Envoi précédent,Facture précédente,Erreur,PDF\n';
    for (const r of rows) {
      csv +=
        [
          r.contract_number,
          ...
          r.previous_sent_at ? new Date(r.previous_sent_at).toLocaleString('fr-FR') : '',
          r.previous_numero_facture || '',
    ...
    reply.header(
      'Content-Disposition',
      `attachment; filename=invoice_sends_${new Date().toISOString().slice(0, 10)}.csv`
    );
    return reply.send('\uFEFF' + csv);
  } catch (error) {
    logger.error('[BotDashboard] exportInvoiceSends:', error.message);
    return reply.code(500).send({ success: false, error: 'Erreur export' });
  }
}
