...
const { canSendToRecipient, reserveQuotaBatch } = require('../services/message.service');
...
async function dispatchBatch(campaign) {
  ...
  // ────────────────────────────────────────────────────────────────
  // 5. 🔥 RÉSERVATION ATOMIQUE DU QUOTA — AVANT LES INSERTS
  // (déplacé ici pour que finalToInsert existe avant d'être utilisé
  // dans les blocs d'insertion plus bas)
  // ────────────────────────────────────────────────────────────────
  const quotaResult = await reserveQuotaBatch(client_id, toInsert.length);

  if (quotaResult.reserved < toInsert.length) {
    logger.warn(
      `[DISPATCH] Quota insuffisant pour campagne ${campaignId}: ` +
      `${quotaResult.reserved}/${toInsert.length} réservés — le reste sera marqué en échec`
    );
  }

  const finalToInsert = toInsert.slice(0, quotaResult.reserved);
  const quotaExceeded = toInsert.slice(quotaResult.reserved);

  if (quotaExceeded.length > 0) {
    await query(
      `UPDATE campaign_contacts SET status = 'skipped', skip_reason = 'QUOTA_EXHAUSTED', updated_at = NOW()
       WHERE id = ANY($1::uuid[])`,
      [quotaExceeded.map(r => r.contactId)]
    );
  }

  if (finalToInsert.length === 0) {
    const lastId = contacts[contacts.length - 1].id;
    if (quotaResult.reserved === 0 && toInsert.length > 0) {
      await query(
        `UPDATE campaigns SET status = 'paused', dispatch_status = 'idle', updated_at = NOW() WHERE id = $1`,
        [campaignId]
      );
      logger.warn(`[DISPATCH] Campagne ${campaignId} mise en pause : quota épuisé`);
      return { processed: contacts.length, lastId, done: true, quotaExhausted: true };
    }
    return { processed: contacts.length, lastId, done: false };
  }

  // 6. INSERT EN MASSE dans messages (1 requête multi-values pour tout le lot)
  ...
  // 7. INSERT EN MASSE dans la table client dédiée
  const clientValues = [];
  const clientParams = [];
  pIdx = 1;
  for (const row of finalToInsert) {
  ...
  // 8. Pousser vers BullMQ (queue WATI) en parallèle contrôlé
  await mapWithConcurrency(finalToInsert, PARALLEL_ENQUEUE, async (row) => {
    const queueData = {
