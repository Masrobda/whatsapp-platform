...
<head>
  ...
  <style>
    ...
    .header { background: linear-gradient(135deg, #2d5016 0%, #8bc34a 100%); color: white; padding: 40px 30px; text-align: center; }
    .logo-img { max-width: 128px; width: 100%; height: auto; margin-bottom: 15px; display: inline-block; }
...
/**
 * Email activation compte agent Socadel (collecte WhatsApp)
 */
function getSocadelAgentActivationTemplate({ full_name, matricule, code }) {
  const logoUrl = 'https://api.numericexport.com/api/logo.png';
  const name = full_name || 'Agent';
  return `
<!DOCTYPE html>
<html>
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <style>
    body { font-family: 'Segoe UI', Arial, sans-serif; background:#f0f5fa; margin:0; padding:0; color:#1e293b; }
    .card { max-width:560px; margin:24px auto; background:#fff; border-radius:16px; overflow:hidden;
            box-shadow:0 12px 40px rgba(15,90,151,0.12); }
    .header { background:linear-gradient(135deg,#1a6fb5,#0f5a97); color:#fff; text-align:center; padding:28px 20px; }
    .header img { height:40px; margin-bottom:10px; }
    .body { padding:28px 24px; }
    .code { font-size:32px; letter-spacing:8px; font-weight:700; color:#0f5a97; text-align:center;
            background:#e8f2fa; border-radius:12px; padding:16px; margin:20px 0; }
    .footer { text-align:center; font-size:12px; color:#64748b; padding:16px; border-top:1px solid #e2e8f0; }
  </style>
</head>
<body>
  <div class="card">
    <div class="header">
      <img src="${logoUrl}" alt="Socadel" />
      <h1 style="margin:0;font-size:20px;">Activation compte collecte</h1>
    </div>
    <div class="body">
      <p>Bonjour <strong>${name}</strong>,</p>
      <p>Votre compte agent Socadel (matricule <strong>${matricule}</strong>) a été créé.</p>
      <p>Voici votre code d’activation (valable <strong>24 heures</strong>) :</p>
      <div class="code">${code}</div>
      <p>Saisissez ce code sur l’écran d’activation de l’application de collecte WhatsApp.</p>
      <p>Si vous n’êtes pas à l’origine de cette demande, ignorez cet email.</p>
      <p>Cordialement,<br/><strong>Socadel — Collecte WhatsApp</strong></p>
    </div>
    <div class="footer">© Socadel / NEXT LTD — Message automatique</div>
  </div>
</body>
</html>`;
}

async function sendSocadelAgentActivationEmail({ email, full_name, matricule, code }) {
  try {
    const info = await transporter.sendMail({
      from: process.env.EMAIL_FROM,
      to: email,
      subject: 'Activation de votre compte collecte Socadel',
      html: getSocadelAgentActivationTemplate({ full_name, matricule, code }),
      text: `Bonjour,\n\nCode d'activation Socadel (24h) : ${code}\nMatricule : ${matricule}\n\nSocadel / NEXT LTD`
    });
    logger.info('Email activation agent Socadel envoyé:', { to: email, messageId: info.messageId });
    return { success: true, messageId: info.messageId };
  } catch (error) {
    logger.error('Erreur email activation agent Socadel:', error);
    throw error;
  }
}

/**
 * Email réinitialisation mot de passe agent Socadel
 * (compte collecte WhatsApp — code à 6 chiffres, valable 15 min)
 */
function getSocadelAgentPasswordResetTemplate({ full_name, matricule, code }) {
  const logoUrl = 'https://api.numericexport.com/api/logo.png';
  const name = full_name || matricule || 'Agent';
  return `
<!DOCTYPE html>
<html>
<head>
  ...
  <style>
    ...
    .code { font-size:32px; letter-spacing:8px; font-weight:700; color:#0f5a97; text-align:center;
            background:#e8f2fa; border-radius:12px; padding:16px; margin:20px 0; }
    .warning { background:#fff7ed; border-left:4px solid #f59e0b; padding:14px 16px;
               border-radius:8px; margin:18px 0; font-size:14px; color:#78350f; }
    .footer { text-align:center; font-size:12px; color:#64748b; padding:16px; border-top:1px solid #e2e8f0; }
  </style>
</head>
<body>
  <div class="card">
    ...
    <div class="body">
      <p>Bonjour <strong>${name}</strong>,</p>
      <p>Vous avez demandé à réinitialiser le mot de passe de votre compte agent Socadel (matricule <strong>${matricule || '—'}</strong>).</p>
      <p>Voici votre code de réinitialisation (valable <strong>15 minutes</strong>) :</p>
      <div class="code">${code}</div>
      <div class="warning">
        ⚠️ Ne partagez ce code avec personne. Notre équipe ne vous le demandera jamais.
      </div>
      <p>Saisissez ce code dans l’écran « Mot de passe oublié » de l’application de collecte, accompagné de votre nouveau mot de passe.</p>
...
async function sendSocadelAgentPasswordResetEmail({ email, full_name, matricule, code }) {
  try {
    const info = await transporter.sendMail({
      from: process.env.EMAIL_FROM,
      to: email,
      subject: 'Réinitialisation de votre mot de passe Socadel Collecte',
      html: getSocadelAgentPasswordResetTemplate({ full_name, matricule, code }),
      text:
        `Bonjour,\n\n` +
        `Code de réinitialisation Socadel (valable 15 minutes) : ${code}\n` +
        `Matricule : ${matricule || '—'}\n\n` +
        `Si vous n'êtes pas à l'origine de cette demande, ignorez cet email.\n\n` +
        `Socadel / NEXT LTD`
    });

    logger.info('Email reset password agent Socadel envoyé:', {
      to: email,
      messageId: info.messageId
    });
    ...
    logger.error('Erreur email reset password agent Socadel:', error);
    throw error;
  }
}
...
module.exports = {
  ...
  testSMTPConnection,
  sendSocadelAgentActivationEmail,
  sendSocadelAgentPasswordResetEmail,
  transporter
};
