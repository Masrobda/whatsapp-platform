...
async function openOrExtendSession({ clientId, phone, channelNumber }) {
  ...
  if (!clientId || !normalizedPhone) {
    logger.warn('[SESSION] openOrExtendSession sans clientId/phone');
    return { success: false };
  }

  // ── 1. Redis SYNCHRONE (chemin chaud, <1–2 ms) ──
  if (redisClient) {
    try {
      await redisClient.set(
        sessionKey(clientId, normalizedPhone),
        '1',
        'EX',
        WINDOW_SECONDS
      );
    } catch (err) {
      logger.error('[SESSION] Redis SET:', err.message);
    }
  }

  // ── 2. Postgres ASYNCHRONE (ne bloque jamais webhook / worker) ──
  setImmediate(() => {
    (async () => {
      try {
        await query(
          `INSERT INTO whatsapp_sessions
             (client_id, recipient_phone, channel_number,
              window_opened_at, window_expires_at, status, last_inbound_at)
        ...
        logger.error('[SESSION] PG upsert async:', err.message);
      }
    })();
  });
