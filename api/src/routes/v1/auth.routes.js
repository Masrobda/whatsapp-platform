...
fastify.post('/register/invitation', {
 ...
 // Route de connexion pour la campagne de collecte
fastify.post('/socadel-login', async (request, reply) => {
  const { username, password } = request.body || {};

  if (
    username === process.env.SOCADEL_USER &&
    password === process.env.SOCADEL_PASS
  ) {
    const token = fastify.jwt.sign(
      { username, role: 'socadel', scope: 'socadel' },
      { expiresIn: '12h' }
    );
