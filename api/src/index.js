...
// ── REDIRECTION VERS CHATBOT POUR RECEVOIR LA FACTURE ──
app.get('/go/facture', async (request, reply) => {
  const { query } = require('./config/database');

  // Tracker le clic avec un type distinct
  query(
    `INSERT INTO bot_link_clicks (ip, user_agent,  clicked_at)
     VALUES ($1, $2, NOW())`,
    [request.ip, request.headers['user-agent']]
  ).catch(() => {});

  // Numéro du chatbot + mot-clé pré-rempli
  const link = `https://wa.me/237688356291?text=${encodeURIComponent('Bonjour')}`;
  return reply.code(302).header('Location', link).send();
});
...
// ── PAIEMENT FACTURE : point d'entrée unique (compatible WhatsApp) ──
  ...
  // 1. Récupérer les détails de la facture depuis la base locale
  const { query } = require('./config/database');
  let factureData;
  try {
    const result = await query(
      `SELECT 
         ...
         ib.lien_pdf,
         c.client_name
       FROM invoices_bot ib
       LEFT JOIN contracts c ON ib.numero_contrat = c.contract_number
       WHERE ib.numero_facture = $1
    ...
    factureData = result.rows[0];
  } catch (error) {
    console.error('Erreur requête locale:', error.message);
